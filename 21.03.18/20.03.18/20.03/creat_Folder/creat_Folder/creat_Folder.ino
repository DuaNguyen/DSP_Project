...
int error[9] = {3,0,0,17,0,6,1,10,0};
byte typeError = 34;
File MyFile;
int _count = 90;//
int sizeHeader = 0;
 ...
 */
void write_SD(unsigned long Position){
  MyFile = SD.open(FileName, FILE_WRITE);
...
//  Serial.println(SD_lineError);
  ...
  for (byte i=0; i<9; i++){
    error[i] = getImfor((i+3)*8+1,(i+4)*8);
